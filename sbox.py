...
sBox[4] = (2, 12, 4, 1, 7, 10, 11, 6, 8, 5, 3, 15, 13, 0, 14, 9, 14, 11, 2, 12,
...
def r_sbox(row, col):
    return (row & 1) ^ ((row & 0b10) << 4) ^ (col << 1)


def test_sbox():
    assert sbox(0) == [0, 0]
    assert sbox(50) == [0b10, 0b1001]
...
def inv_sbox(n, a):
    ...
    while row < 4:
        col = 0
        while col < 16 and s[row * 16 + col] != a:
            col += 1
        ls.append(r_sbox(row, col))
        row += 1
    return ls
